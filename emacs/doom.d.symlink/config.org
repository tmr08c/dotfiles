...
DOOM [[https://github.com/doomemacs/doomemacs/commit/68ec9e9c86009623ced5849343534bced2e6fcd3][removed `jk`]] as the default escape sequence, but [[https://tmr08c.github.io/2022/10/alt-escape-vim/][I found I liked it]], so this re-enabled it.
...
*** Fix quit keybinding :workaround:

For some reason, the evil keybinding from Doom's config isn't being applied properly. Let's explicitly set it:
...
We should periodically check if this is still necessary.

*** Saved Filters

Elfeed supports saving filters using Emacs' built-in bookmark system, but we can also create quick shortcuts for commonly used filters based on the tags in our =elfeed.org= file.

#+begin_src elisp
(after! elfeed
  ;; Quick filter functions for common searches
  (defun tr/elfeed-filter-all ()
    "Show all unread entries from the last 3 months"
    (interactive)
    (elfeed-search-set-filter "@3-month-ago +unread"))

  (defun tr/elfeed-filter-jobs ()
    "Show unread job postings"
    (interactive)
    (elfeed-search-set-filter "@3-month-ago +unread +jobs"))

  (defun tr/elfeed-filter-emacs ()
    "Show unread Emacs-related posts"
    (interactive)
    (elfeed-search-set-filter "@3-month-ago +unread +emacs"))

  (defun tr/elfeed-filter-programming ()
    "Show unread programming posts"
    (interactive)
    (elfeed-search-set-filter "@3-month-ago +unread +programming"))

  (defun tr/elfeed-filter-productivity ()
    "Show unread productivity posts"
    (interactive)
    (elfeed-search-set-filter "@3-month-ago +unread +producivity"))

  (defun tr/elfeed-filter-coffee ()
    "Show unread coffee posts"
    (interactive)
    (elfeed-search-set-filter "@3-month-ago +unread +coffee"))

  (defun tr/elfeed-filter-today ()
    "Show all posts from today"
    (interactive)
    (elfeed-search-set-filter "@1-day-ago"))

  (defun tr/elfeed-filter-week ()
    "Show unread posts from this week"
    (interactive)
    (elfeed-search-set-filter "@1-week-ago +unread"))

  ;; Keybindings for quick filters and bookmark management
  (map! :map elfeed-search-mode-map
        :localleader
        (:prefix ("f" . "filters")
         :desc "All unread" "a" #'tr/elfeed-filter-all
         :desc "Jobs" "j" #'tr/elfeed-filter-jobs
         :desc "Emacs" "e" #'tr/elfeed-filter-emacs
         :desc "Programming" "p" #'tr/elfeed-filter-programming
         :desc "Productivity" "P" #'tr/elfeed-filter-productivity
         :desc "Coffee" "c" #'tr/elfeed-filter-coffee
         :desc "Today" "t" #'tr/elfeed-filter-today
         :desc "This week" "w" #'tr/elfeed-filter-week)
        :localleader
        (:prefix ("b" . "bookmarks")
         :desc "Save filter bookmark" "s" #'bookmark-set
         :desc "Jump to filter bookmark" "j" #'bookmark-jump
...
**** LSP
***** Explorer :experiement:
  Trying to be an early adopter (as of <2025-11-19 Wed>) of the official Elixir LSP, [[https://github.com/elixir-lang/expert][Expert]].

  For now, this requires pulling down a nightly release. To make this as bit easier, and to potentially work with tagged version releases in the future, let's set up some helpers to find and fetch the proper release from GitHub. We will even check if we can have Expert on our path and suggest downloading it if we don't.

#+begin_src elisp
(defcustom tr/explorer-ls-version "latest"
  "Version of Explorer language server to download.
Can be a specific version string (e.g., \"v0.1.0\") or \"latest\"."
  :type 'string
  :group 'elixir)

(defun tr/get-explorer-download-url ()
  "Return download URL and binary name for current system."
  (let* ((version (if (string= tr/explorer-ls-version "latest")
                      "nightly"
                      (concat "tags/" tr/explorer-ls-version)))
         (base-url (format "https://github.com/elixir-lang/expert/releases/download/%s/" version)))
    (cond
     ;; macOS ARM (M1/M2/M3)
     ((and (eq system-type 'darwin)
           (string-match-p "arm64\\|aarch64" (shell-command-to-string "uname -m")))
      (list :binary "expert_darwin_arm64" :url (concat base-url "expert_darwin_arm64")))

     ;; macOS Intel
     ((eq system-type 'darwin)
      (list :binary "expert_darwin_amd64" :url (concat base-url "expert_darwin_amd64")))

     ;; Windows
     ((eq system-type 'windows-nt)
      (list :binary "expert_windows_amd64.exe" :url (concat base-url "expert_windows_amd64.exe")))

     ;; Linux (default)
     (t
      (list :binary "expert_linux_amd64" :url (concat base-url "expert_linux_amd64"))))))

(defvar tr/explorer-install-dir (expand-file-name "~/.local/bin")
  "Directory where the Expert binary is installed.")

;; Add install dir to exec-path at load time so executable-find works immediately.
(add-to-list 'exec-path tr/explorer-install-dir)

(defun tr/ensure-explorer-installed ()
  "Check if Explorer is installed and prompt to install if not."
  (unless (or (executable-find "expert_linux_amd64")
              (executable-find "expert_darwin_amd64")
              (executable-find "expert_darwin_arm64")
              (executable-find "expert_windows_amd64"))
    (when (y-or-n-p "Explorer language server not found. Install it?")
      (let* ((download-info (tr/get-explorer-download-url))
             (binary-name (plist-get download-info :binary))
             (download-url (plist-get download-info :url))
             (binary-path (expand-file-name binary-name tr/explorer-install-dir)))
        (unless (file-directory-p tr/explorer-install-dir)
          (make-directory tr/explorer-install-dir t))
        (message "Downloading Explorer...")
        (url-copy-file download-url binary-path t)
        (set-file-modes binary-path #o755)
        (message "Explorer installed to %s." binary-path)))))

(defun tr/update-explorer ()
  "Update Explorer language server to the latest nightly build."
  (interactive)
  (let* ((download-info (tr/get-explorer-download-url))
         (binary-name (plist-get download-info :binary))
         (download-url (plist-get download-info :url))
         (binary-path (expand-file-name binary-name tr/explorer-install-dir)))
    (when (y-or-n-p "Update Explorer to latest nightly build? ")
      (message "Downloading latest Explorer...")
      (when (file-exists-p binary-path)
        (delete-file binary-path))
      (url-copy-file download-url binary-path t)
      (set-file-modes binary-path #o755)
      (message "Explorer updated to latest nightly build at %s" binary-path))))

;; Hook to check for Explorer when opening Elixir files
(add-hook 'elixir-mode-hook #'tr/ensure-explorer-installed)
(add-hook 'elixir-ts-mode-hook #'tr/ensure-explorer-installed)
#+end_src

Now, let's tell ~eglot~ to use our expert binary.
