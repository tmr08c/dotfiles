  ...
  (unless (or (executable-find "expert_linux_amd64")
    ...
    (when (y-or-n-p "Explorer language server not found. Install it?")
      (let* ((download-info (tr/get-explorer-download-url))
             ...
             (install-dir (expand-file-name "~/.local/bin"))
             (binary-path (expand-file-name binary-name install-dir)))
        ...
        (message "Explorer installed to %s. Please ensure %s is in your PATH." binary-path install-dir)))))
