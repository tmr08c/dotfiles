...
** Window Management
*** Buffer switching
Treat manual buffer switching (i.e., ~SPC B B~) the same as automatic buffer switching. Suggested in [[https://www.masteringemacs.org/article/demystifying-emacs-window-manager][this Mastering Emacs article]].

#+begin_src elisp
;; this may be causing problems with elfeed
;; (setq switch-to-buffer-obey-display-actions 'nil)
#+end_src
*** Configure window display preferences

See [[id:0918299e-df89-4a38-a20c-8821460c7f8a][emacs window management]] for more.

#+begin_src elisp
(add-to-list 'display-buffer-alist
          `(,(rx (| "*compilation*" "*grep*" "*exunit-compilation*\*"))
...
(after! org
  ...
  (add-to-list 'org-capture-templates
             `("ml" "Weekly IT Leadership" entry (file+datetree ,(expand-file-name "work/cpower/weekly-it-leadership.org" org-directory)) "*** Prep\n- %?\n*** Meeting Notes" :jump-to-capture t))
...
**** Templates
Override the default capture template to include fields to track when a file was created and updated.
...
To update the =last_modified= field, we leverage the built-in =time-stamp= function.
...
I may check my feeds across multiple machines. To keep the database in sync, I want to safe it before we close elfeed. There is [[https://github.com/skeeto/elfeed/issues/169][alternative options]] such as writing a custom function to use for quitting or mention of hooking into the update function. Since my current flow is have elfeed reading sessions, hooking into quitting seems to be a good sync point for me (for now).
